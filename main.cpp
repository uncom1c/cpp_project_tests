...
#pragma endregion
//Свапы
#pragma region OftenUsFunctions
...
// }
#pragma endregion
//Функции сортировки, запоминание шагов сортировки
#pragma region Sorts
// //СОРТИРОВКА ВЫБОРОМ
...
//     }
// }
// //ПИРАМИИДАЛЬНАЯ СОРТИРОВКА
// void downHeap(double * a, long k, long n, std::vector <std::pair <int, int > >& vec)
// {
...
//     a[k] = new_elem;
// }
// void heapSort(double* a, int size, std::vector <std::pair <int, int > >& vec)
// {
//     int i;

//     // строим пирамиду 
//     for (i = size / 2 - 1; i >= 0; --i)
//         downHeap(a, i, size - 1, vec);
...
#pragma region SetCoordinatesForSprites
    ...
    double listXS8[100];
    double listYS8[100];
    double listXS8Res[100];
    double listYS8Res[100];
    double listXS9[100];
    double listYS9[100];
    for (int S = 0; S < 9; S++) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        listXS8Res[i] = listX[7][numbers[i]];
        listYS8Res[i] = listY[7][numbers[i]];
...
#pragma region SortsOn
    ...
    std::vector<std::pair<double, double>> iterS8Copy;
...
#pragma region Checking
 /*   for (int i = 0; i < 100; i++) {
    ...
    for (int i = 0; i < iterS8.size(); i++) {
        std::cout << iterS8[i].first << "," << iterS8[i].second << std::endl;
        std::cout << iterS8Copy[i].first << "," << iterS8Copy[i].second << std::endl;
    }
    */
#pragma endregion
   ...
   //рисую этот текст
    ...
    while (Window.isOpen())
    {
        ...
        if (duration > 0.05f)
        {
            ...
            if (i < iterS1.size()) {
                if (iterS1[i].first != -1) {
                    swap(&listX[0][iterS1[i].first], &listX[0][iterS1[i].second]);
                    swap(&listY[0][iterS1[i].first], &listY[0][iterS1[i].second]);
                }
                i++;
            }
            if (j < iterS2.size()) {
                if (iterS2[j].first != -1) {
                    swap(&listX[1][iterS2[j].first], &listX[1][iterS2[j].second]);
                    swap(&listY[1][iterS2[j].first], &listY[1][iterS2[j].second]);
                }
                j++;
            }
            if (k < iterS3.size()) {
                if (iterS3[k].first != -1) {
                    swap(&listX[2][iterS3[k].first], &listX[2][iterS3[k].second]);
                    swap(&listY[2][iterS3[k].first], &listY[2][iterS3[k].second]);
                }
                k++;
            }
            if (x < iterS4.size()) {
                if (iterS4[x].first != -1) {
                    swap(&listX[3][iterS4[x].first], &listX[3][iterS4[x].second]);
                    swap(&listY[3][iterS4[x].first], &listY[3][iterS4[x].second]);
                }
                x++;
            }
            if (z < iterS6.size()) {
                if (iterS6[z].first != -1) {
                    swap(&listX[5][iterS6[z].first], &listX[5][iterS6[z].second]);
                    swap(&listY[5][iterS6[z].first], &listY[5][iterS6[z].second]);
                }
                z++;
            }
            if (c < iterS7.size()) {
                if (iterS7[c].first != -1) {
                    swap(&listX[6][iterS7[c].first], &listX[6][iterS7[c].second]);
                    swap(&listY[6][iterS7[c].first], &listY[6][iterS7[c].second]);
                }
                c++;
            }
            if (v < iterS9.size()) {
                if (iterS9[v].first != -1) {
                    swap(&listX[8][iterS9[v].first], &listX[8][iterS9[v].second]);
                    swap(&listY[8][iterS9[v].first], &listY[8][iterS9[v].second]);
                }
                v++;
            }
            if (d < iterS8Copy.size()) {
                listX[7][iterS8[d].second] = iterS8Copy[iterS8[d].first].first;
                listY[7][iterS8[d].second] = iterS8Copy[iterS8[d].first].second;
                d++;
            }
        }
